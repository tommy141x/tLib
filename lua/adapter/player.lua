-- ── NUI callback diagnostic (remove after testing) ──────────────────────────
if type(RegisterNuiCallbackType) == 'function' then
    print('[tLib] RegisterNuiCallbackType exists: ' .. type(RegisterNuiCallbackType))
    RegisterNuiCallbackType('testPing')
    AddEventHandler('__cfx_nui:testPing', function(body, cb)
        print('[tLib] *** testPing FIRED ***')
        cb(json.encode({ ok = true }))
    end)
    print('[tLib] testPing registered via RegisterNuiCallbackType + AddEventHandler')
else
    print('[tLib] WARNING: RegisterNuiCallbackType is NIL — type=' .. type(RegisterNuiCallbackType))
end

if type(RegisterNUICallback) == 'function' then
    RegisterNUICallback('testPing2', function(data, ret)
        print('[tLib] *** testPing2 FIRED via RegisterNUICallback ***')
        ret('ok')
    end)
    print('[tLib] testPing2 registered via RegisterNUICallback')
else
    print('[tLib] WARNING: RegisterNUICallback is NIL')
end
