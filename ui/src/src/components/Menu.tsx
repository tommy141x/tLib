  ...
  onNuiEvent<{ dir: string }>("navigate", ({ dir }) => {
    setMenu((prev) => {
      ...
      if (dir === "up") {
        do {
          idx = (idx - 1 + len) % len;
        } while (prev.items[idx]?.type === "separator" && idx !== prev.focusedIndex);
      } else {
        do {
          idx = (idx + 1) % len;
        } while (prev.items[idx]?.type === "separator" && idx !== prev.focusedIndex);
                  ...
                  return (
                    <div
                      ...
                      style={
                        item.disabled
                          ? undefined
                          : { "min-height": m().itemHeight ? `${m().itemHeight}px` : undefined }
                      }
                    >
                      ...
                      <Show when={item.type === "checkbox"}>
                        <div
                          class={`w-4 h-4 rounded border flex items-center justify-center text-xs ${item.checked ? "bg-primary border-primary text-primary-foreground" : "border-border"}`}
                        >
                          {item.checked ? "✓" : ""}
                        </div>
                      </Show>
