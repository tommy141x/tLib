...
export function registerBridge(getServer: () => RealtimeServer, cfg: RealtimeConfig): void {
	...
	fExports("rtBroadcastEvent", (roomId: unknown, type: unknown, payload: unknown, excludePid?: unknown) => {
	...
	fExports("rtStartControl", (licenseIn: unknown, tokenEndpointIn: unknown) => {
		if (typeof licenseIn !== "string" || licenseIn.length === 0) return { ok: false, error: "license" };
		if (typeof tokenEndpointIn !== "string" || tokenEndpointIn.length === 0) return { ok: false, error: "tokenEndpoint" };
		startControl(
			{ license: licenseIn, tokenEndpoint: tokenEndpointIn },
			(level, msg) => {
