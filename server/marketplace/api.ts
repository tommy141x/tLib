...
export async function listItems(params: ListParams): Promise<{ items: MarketplaceItem[]; nextCursor: string | null }> {
...
export async function downloadBundle(slug: string, version: string): Promise<BundleDownload> {
	...
	const rawSize = resp.headers.get("x-marketplace-size-bytes") ?? resp.headers.get("content-length");
	if (!rawHash) {
